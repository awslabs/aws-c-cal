...
static int s_ecdsa_p256_test_uneven_r_s(struct aws_allocator *allocator, void *ctx) {
    (void)ctx;
    ...
    uint8_t message[] = {
        0xf1, 0x6e, 0xed, 0x63, 0x0f, 0xc2, 0xef, 0x0a, 0x74, 0x5f, 0x2f, 0x52, 0x37, 0x44, 0x05,
        0x6d, 0x2e, 0x96, 0x56, 0x07, 0x56, 0xa2, 0x80, 0x88, 0xc0, 0x24, 0x12, 0x1e, 0x6d, 0x91,
        0xa3, 0x5f,
    };

    uint8_t signature[] = {
        0x30, 0x44, 0x02, 0x21, 0x00, 0xa3, 0xbc, 0xfe, 0xe8, 0xfa, 0x98, 0x6c, 0x2e, 0x62, 0x5b,
        0x7b, 0x2e, 0xeb, 0xa6, 0x6d, 0xc0, 0x1a, 0x8e, 0x75, 0x15, 0x48, 0xfe, 0x9c, 0x2e, 0xa2,
        0x3e, 0x84, 0xc3, 0xb2, 0xd5, 0x3f, 0x37, 0x02, 0x1f, 0x06, 0x01, 0x98, 0xaf, 0xf7, 0x18,
        0x90, 0xee, 0x95, 0x37, 0x52, 0x4d, 0xb3, 0xdf, 0xd5, 0xd0, 0x9f, 0xc5, 0x7c, 0x43, 0xe2, 
        0xd3, 0xf7, 0x76, 0x58, 0xf3, 0x77, 0x11, 0x48, 0x9a, 0x37
    };

    struct aws_byte_cursor message_cur = aws_byte_cursor_from_array(message, AWS_ARRAY_SIZE(message));
    struct aws_byte_cursor sig_cur = aws_byte_cursor_from_array(signature, AWS_ARRAY_SIZE(signature));

    ASSERT_SUCCESS(aws_ecc_key_pair_verify_signature(key_pair, &message_cur, &sig_cur));

    aws_ecc_key_pair_release(key_pair);

    return AWS_OP_SUCCESS;
}
AWS_TEST_CASE(ecdsa_p256_test_uneven_r_s, s_ecdsa_p256_test_uneven_r_s);

static int s_ecdsa_p256_randomize(struct aws_allocator *allocator, void *ctx) {
    (void)ctx;
    ...
    for (int i = 0; i < 10000; ++i) {
        ...
        aws_byte_buf_init(&buf, allocator, 100);
        ASSERT_SUCCESS(aws_device_random_buffer(&buf));

        struct aws_byte_cursor message_input = aws_byte_cursor_from_buf(&buf);
        uint8_t hash[AWS_SHA256_LEN];
        AWS_ZERO_ARRAY(hash);
        struct aws_byte_buf hash_value = aws_byte_buf_from_empty_array(hash, sizeof(hash));
        aws_sha256_compute(allocator, &message_input, &hash_value, 0);
        ...
        struct aws_byte_cursor signature = aws_byte_cursor_from_buf(&signature_buf);
        ...
        size_t size_one = coordinate.len;
        ...
        size_t size_two = coordinate.len;

        if (size_one != 32 || size_two != 32 ) {
            ++uneven;
            //AWS_LOGF_DEBUG(0, "R S sizes %zu %zu", size_one, size_two);
            struct aws_byte_buf hash_hex_buf;
            aws_byte_buf_init(&hash_hex_buf, allocator, 1024);
            ASSERT_SUCCESS(aws_hex_encode(&hash_cur, &hash_hex_buf));
            struct aws_byte_buf sig_hex_buf;
            aws_byte_buf_init(&sig_hex_buf, allocator, 1024);
            ASSERT_SUCCESS(aws_hex_encode(&signature, &sig_hex_buf));
            AWS_LOGF_DEBUG(0, "Failed hash " PRInSTR, AWS_BYTE_BUF_PRI(hash_hex_buf));
            AWS_LOGF_DEBUG(0, "Failed signature " PRInSTR, AWS_BYTE_BUF_PRI(sig_hex_buf)); 
            ASSERT_FALSE(true);
        }
    ...
    ASSERT_INT_EQUALS(uneven, 0);

    aws_ecc_key_pair_release(key_pair);

    return AWS_OP_SUCCESS;
}
AWS_TEST_CASE(ecdsa_p256_randomize, s_ecdsa_p256_randomize);
