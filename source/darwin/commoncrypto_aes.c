...
struct aws_symmetric_cipher *aws_aes_cbc_256_new(struct aws_allocator *allocator, const struct aws_byte_cursor* key, const struct aws_byte_cursor *iv) {
    ...
    if (iv) {
        ...
        aws_symmetric_cipher_generate_initialization_vector(AWS_AES_256_CIPHER_BLOCK_SIZE, false, &cc_cipher->cipher_base.iv);
    }

    CCCryptorStatus status = CCCryptorCreateWithMode(kCCEncrypt, kCCModeCBC, kCCAlgorithmAES, ccPKCS7Padding,
                                                     cc_cipher->cipher_base.iv.buffer, cc_cipher->cipher_base.key.buffer, cc_cipher->cipher_base.key.len,
                                                     NULL, 0, 0, 0, &cc_cipher->encryptor_handle);

    status |= CCCryptorCreateWithMode(kCCDecrypt, kCCModeCBC, kCCAlgorithmAES, ccPKCS7Padding,
                                      cc_cipher->cipher_base.iv.buffer, cc_cipher->cipher_base.key.buffer, cc_cipher->cipher_base.key.len,
                                      NULL, 0, 0, 0, &cc_cipher->decryptor_handle);
...
struct aws_symmetric_cipher *aws_aes_ctr_256_new(struct aws_allocator *allocator, const struct aws_byte_cursor* key, const struct aws_byte_cursor *iv) {
    ...
    if (iv) {
        ...
        aws_byte_buf_init(&cc_cipher->cipher_base.iv, allocator, AWS_AES_256_CIPHER_BLOCK_SIZE);
        aws_symmetric_cipher_generate_initialization_vector(AWS_AES_256_CIPHER_BLOCK_SIZE, true, &cc_cipher->cipher_base.iv);
    }

    CCCryptorStatus status = CCCryptorCreateWithMode(kCCEncrypt, kCCModeCTR, kCCAlgorithmAES, ccPKCS7Padding,
                                                     cc_cipher->cipher_base.iv.buffer, cc_cipher->cipher_base.key.buffer, cc_cipher->cipher_base.key.len,
                                                     NULL, 0, 0, kCCModeOptionCTR_BE, &cc_cipher->encryptor_handle);

    status |= CCCryptorCreateWithMode(kCCDecrypt, kCCModeCTR, kCCAlgorithmAES, ccPKCS7Padding,
                                      cc_cipher->cipher_base.iv.buffer, cc_cipher->cipher_base.key.buffer, cc_cipher->cipher_base.key.len,
                                      NULL, 0, 0, kCCModeOptionCTR_BE, &cc_cipher->decryptor_handle);
...
struct aws_symmetric_cipher *aws_aes_gcm_256_new(struct aws_allocator *allocator, const struct aws_byte_cursor* key, const struct aws_byte_cursor *iv, const struct aws_byte_cursor* aad) {
    ...
    if (iv) {
        ...
        /* GCM IVs are kind of a hidden implementation detail. 4 are reserved by the system for long running stream blocks. */
        ...
        aws_symmetric_cipher_generate_initialization_vector(AWS_AES_256_CIPHER_BLOCK_SIZE - 4, false, &cc_cipher->cipher_base.iv);
    }
    ...
    CCCryptorStatus status = CCCryptorCreateWithMode(kCCEncrypt, kCCModeGCM, kCCAlgorithmAES, ccPKCS7Padding,
                                                     NULL, cc_cipher->cipher_base.key.buffer, cc_cipher->cipher_base.key.len,
                                                     NULL, 0, 0, kCCModeOptionCTR_BE, &cc_cipher->encryptor_handle);

#ifdef MAC_13_AVAILABLE
    status |= CCCryptorGCMSetIV(cc_cipher->encryptor_handle, cc_cipher->cipher_base.iv.buffer, cc_cipher->cipher_base.iv.len);
#else
    status |= CCCryptorGCMAddIV(cc_cipher->encryptor_handle, cc_cipher->cipher_base.iv.buffer, cc_cipher->cipher_base.iv.len);
#endif
    if (aad && aad->len)
    {
        status |= CCCryptorGCMAddAAD(cc_cipher->encryptor_handle, cc_cipher->cipher_base.aad.buffer, cc_cipher->cipher_base.aad.len);
    }

    status |= CCCryptorCreateWithMode(kCCDecrypt, kCCModeGCM, kCCAlgorithmAES, ccPKCS7Padding,
                                      NULL, cc_cipher->cipher_base.key.buffer, cc_cipher->cipher_base.key.len,
                                      NULL, 0, 0, kCCModeOptionCTR_BE, &cc_cipher->decryptor_handle);

#ifdef MAC_13_AVAILABLE
    status |= CCCryptorGCMSetIV(cc_cipher->decryptor_handle, cc_cipher->cipher_base.iv.buffer, cc_cipher->cipher_base.iv.len);
#else
    status |= CCCryptorGCMAddIV(cc_cipher->decryptor_handle, cc_cipher->cipher_base.iv.buffer, cc_cipher->cipher_base.iv.len);
#endif
    if (aad && aad->len)
    {
        status |= CCCryptorGCMAddAAD(cc_cipher->decryptor_handle, cc_cipher->cipher_base.aad.buffer, cc_cipher->cipher_base.aad.len);
    }
