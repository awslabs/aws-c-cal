...
static inline int s_verify_hmac_test_case(struct aws_allocator *allocator,
  ...
  for (size_t i = 1; i < input->len; ++i) {
    ...
    struct aws_byte_buf output_buf =
        aws_byte_buf_from_array(output, expected->len);
    output_buf.len = 0;
    ...
    ASSERT_BIN_ARRAYS_EQUALS(expected->ptr, expected->len, output_buf.buffer,
                             output_buf.len);
...
static inline int s_verify_hash_test_case(struct aws_allocator *allocator,
                                          struct aws_byte_cursor *input,
                                          struct aws_byte_cursor *expected,
