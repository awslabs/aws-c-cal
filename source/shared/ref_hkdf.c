...
int aws_hkdf_derive_impl(
    ...
    struct aws_byte_buf prk_buf = aws_byte_buf_from_array(prk, MAX_HMAC_SIZE);

    if (!s_hkdf_extract(allocator, hash_type, salt, ikm, &prk_buf)) {
        return AWS_OP_ERR;
    }

    struct aws_byte_cursor prk_cur = aws_byte_cursor_from_buf(&prk_buf);

    return s_hkdf_expand(allocator, hash_type, prk_cur, info, out_buf, length);
}
