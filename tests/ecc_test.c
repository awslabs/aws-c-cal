...
static int s_ecdsa_p256_test_pub_key_derivation_fn(struct aws_allocator *allocator, void *ctx) {
    ...
    ASSERT_BIN_ARRAYS_EQUALS(x, sizeof(x), pub_x.ptr, pub_x.len);
    ...
    ASSERT_BIN_ARRAYS_EQUALS(y, sizeof(y), private_key_pair->pub_y.buffer, private_key_pair->pub_y.len);
    ASSERT_BIN_ARRAYS_EQUALS(y, sizeof(y), pub_y.ptr, pub_y.len);
...
static int s_ecdsa_p256_test_key_gen_export_fn(struct aws_allocator *allocator, void *ctx) {
    ...
    struct aws_ecc_key_pair *signing_key =
        aws_ecc_key_pair_new_from_private_key(allocator, AWS_CAL_ECDSA_P256, &priv_d);
    ASSERT_NOT_NULL(signing_key);
...
static int s_ecdsa_p256_test_import_asn1_key_pair_public_only_fn(struct aws_allocator *allocator, void *ctx) {
    (void)ctx;
...
AWS_TEST_CASE(ecdsa_p256_test_import_asn1_key_pair_public_only, s_ecdsa_p256_test_import_asn1_key_pair_public_only_fn)
...
static int s_ecdsa_p256_test_signature_format_fn(struct aws_allocator *allocator, void *ctx) {
    (void)ctx;
...
AWS_TEST_CASE(ecdsa_p256_test_signature_format, s_ecdsa_p256_test_signature_format_fn)
