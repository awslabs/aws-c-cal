...
static int s_byte_buf_decoded_from_base64_cur(struct aws_allocator *allocator, 
    struct aws_byte_cursor cur, struct aws_byte_buf *out) {
...
static int s_rsa_encryption_roundtrip_helper(struct aws_allocator *allocator,
    struct aws_rsa_key_pair *key_pair, enum aws_rsa_encryption_algorithm algo) {
...
static const char *TEST_PKCS1_RSA_PRIVATE_KEY_2048 =
"MIIEpQIBAAKCAQEAnt/K3mvMgPQcplTgDHrItQPvXiYpUFMt5nIJbZV820Zj4n4G"
...
static int s_rsa_encryption_roundtrip_from_user(struct aws_allocator *allocator, enum aws_rsa_encryption_algorithm algo) {
    struct aws_byte_buf key_buf;
    ASSERT_SUCCESS(s_byte_buf_decoded_from_base64_cur(allocator,
        aws_byte_cursor_from_c_str(TEST_PKCS1_RSA_PRIVATE_KEY_2048), &key_buf));

    struct aws_rsa_key_pair *key_pair = aws_rsa_key_pair_new_from_private_key_pkcs1(allocator,
...
static const char *TEST_PKCS1_RSA_PRIVATE_KEY_1024 =
"MIICXAIBAAKBgQCVGG9c6uBIpv4wQMB4PJpkaUjEqa6TW5v8ebStMCnICWfpUubs"
...
static const char *TEST_PKCS1_RSA_PUBLIC_KEY_1024 =
"MIGJAoGBAJUYb1zq4Eim/jBAwHg8mmRpSMSprpNbm/x5tK0wKcgJZ+lS5ux/qevmc/"
...
*/
static const char *TEST_RSA_SIGNATURE_PKCS1 = 
"Gqu9pLlPvSFIW+5ZFo9ZCxMmPR8LnAeiuYir5CfNTyraF2VPksRnCKtS6i98nwPUqzlPr"
"TYJ45P3c94lQIQD3SVJ3XMSAyAEWTE2pcj0F/oPzzxLcXK9cyv2Iphe4XuBjWCOVdHgFg"
"rD/yAA8b+B94AqE9U/B2+k9/C3Bz2YApo=";

static int s_rsa_signing_roundtrip_helper(struct aws_allocator *allocator,
    struct aws_rsa_key_pair *key_pair_private,
    struct aws_rsa_key_pair *key_pair_public,
    enum aws_rsa_signing_algorithm algo,
...
static int s_rsa_signing_roundtrip_from_user(struct aws_allocator *allocator,
    enum aws_rsa_signing_algorithm algo, const char *expected_signature) {

    struct aws_byte_buf private_key_buf;
    ASSERT_SUCCESS(s_byte_buf_decoded_from_base64_cur(allocator,
        aws_byte_cursor_from_c_str(TEST_PKCS1_RSA_PRIVATE_KEY_1024), &private_key_buf));
    struct aws_rsa_key_pair *key_pair_private = aws_rsa_key_pair_new_from_private_key_pkcs1(allocator,
        aws_byte_cursor_from_buf(&private_key_buf));
    ASSERT_NOT_NULL(key_pair_private);

    struct aws_byte_buf public_key_buf;
    ASSERT_SUCCESS(s_byte_buf_decoded_from_base64_cur(allocator,
        aws_byte_cursor_from_c_str(TEST_PKCS1_RSA_PUBLIC_KEY_1024), &public_key_buf));
    struct aws_rsa_key_pair *key_pair_public = aws_rsa_key_pair_new_from_public_key_pkcs1(allocator,
...
static int s_rsa_signing_roundtrip_pkcs1_sha256_from_user(struct aws_allocator *allocator, void *ctx) {
    ...
    ASSERT_SUCCESS(s_rsa_signing_roundtrip_from_user(allocator, 
        AWS_CAL_RSA_SIGNATURE_PKCS1_5_SHA256, TEST_RSA_SIGNATURE_PKCS1));
...
static int s_rsa_private_pkcs1_der_parsing(struct aws_allocator *allocator, void *ctx) {
    ...
    static uint8_t n[] = {
        0x95, 0x18, 0x6f, 0x5c, 0xea, 0xe0, 0x48, 0xa6, 0xfe, 0x30, 0x40, 0xc0, 0x78, 0x3c, 0x9a,
    ...
    static uint8_t d[] = {
        0x4f, 0x42, 0x11, 0x19, 0x4d, 0x06, 0xf3, 0xb8, 0x4a, 0x52, 0x2e, 0x69, 0xe2, 0xc1, 0x1c,
    ...
    static uint8_t p[] = {
        0xd5, 0x14, 0x2e, 0xea, 0x1f, 0x79, 0xbe, 0x80, 0x7a, 0x04, 0xc6, 0x7c, 0xfe, 0xb8, 0x84, 
        0x75, 0x1f, 0xbf, 0x0d, 0x42, 0xdd, 0xec, 0x6b, 0x67, 0x27, 0x64, 0x36, 0xb4, 0xba, 0x2f, 
        0xc4, 0xc3, 0xe5, 0x00, 0x49, 0x09, 0x43, 0x0d, 0x8f, 0xf1, 0xa8, 0x01, 0x98, 0x6b, 0xe4, 
        0x00, 0x85, 0x00, 0x4c, 0x66, 0x85, 0x89, 0x2c, 0xbe, 0x80, 0x68, 0xda, 0xe3, 0x16, 0xdc, 
        0x72, 0x2a, 0xbf, 0x93
    };

    static uint8_t q[] = {
        0xb3, 0x20, 0xd3, 0x7f, 0x02, 0x4f, 0x15, 0x14, 0xef, 0xa9, 0x43, 0x4a, 0x28, 0x8b, 0x05, 
        0x1e, 0x53, 0x65, 0xcc, 0x37, 0x25, 0x21, 0x21, 0xdc, 0x22, 0x2d, 0x34, 0x49, 0x3d, 0x9b, 
        0x0c, 0xaa, 0x31, 0xee, 0x94, 0xb7, 0x6a, 0xe9, 0x04, 0x76, 0x4c, 0xad, 0x54, 0x87, 0xab, 
        0x20, 0xab, 0x41, 0x31, 0xdb, 0x39, 0x82, 0x36, 0x19, 0xc9, 0xb0, 0x00, 0x5f, 0x47, 0x02, 
        0xec, 0x31, 0xe5, 0xa1
    };

    static uint8_t dmp1[] = {
        0x6e, 0xde, 0x55, 0x9a, 0xd4, 0x4d, 0xd0, 0x65, 0xa1, 0x17, 0xa2, 0xff, 0x9e, 0xb9, 0xfb,
        0x91, 0xc5, 0xb6, 0xee, 0xfb, 0x7f, 0xe5, 0x67, 0xed, 0x00, 0x7e, 0x10, 0x48, 0xf1, 0x27, 
        0xb1, 0xc6, 0x0b, 0xfd, 0x9e, 0x03, 0x07, 0xb4, 0xa2, 0xd5, 0x26, 0x89, 0xcd, 0xf9, 0x35, 
        0x65, 0x5f, 0xce, 0xcb, 0xf1, 0xfc, 0x41, 0x67, 0xda, 0xa7, 0x8d, 0xe7, 0x76, 0x89, 0x8a, 
        0xa2, 0x8c, 0xc3, 0xa9
    };

    static uint8_t dmq1[] = {
        0x5f, 0xd9, 0x64, 0xd2, 0x45, 0x6e, 0x03, 0x9d, 0xdc, 0x4f, 0xfa, 0x0d, 0xa8, 0x7c, 0x1b, 
        0x15, 0x0b, 0x13, 0x21, 0x9f, 0x30, 0x62, 0xf6, 0x37, 0x50, 0xa2, 0x57, 0xa4, 0x09, 0x2e, 
        0xfa, 0x2b, 0xe7, 0xe3, 0xfd, 0xf7, 0xd7, 0x61, 0x30, 0x5e, 0x0c, 0xd1, 0x82, 0xb5, 0xc5, 
        0xc0, 0x09, 0x63, 0x2d, 0x46, 0x5d, 0x13, 0x6b, 0xd7, 0x88, 0xb0, 0x80, 0xc2, 0x7a, 0xc0, 
        0xf9, 0x9a, 0x09, 0x81
    };

    static uint8_t iqmp[] = {
        0x8f, 0xcf, 0xd5, 0x74, 0xea, 0x10, 0x05, 0x2b, 0xa3, 0x96, 0x83, 0x0f, 0xae, 0x09, 0xbb,
        0x06, 0xef, 0x0a, 0x96, 0xc0, 0x2f, 0xc9, 0x24, 0xc0, 0x6c, 0xdf, 0xeb, 0x0c, 0xeb, 0x74, 
        0x5f, 0x9f, 0xd0, 0x25, 0xca, 0x91, 0x31, 0x94, 0x10, 0x6e, 0xc1, 0x19, 0x6a, 0x82, 0xaf, 
        0xdc, 0xbd, 0x9c, 0x1b, 0x7d, 0x2a, 0xec, 0x8d, 0xd5, 0x59, 0x4d, 0x6f, 0x38, 0x89, 0xa7, 
        0xe5, 0x1c, 0x29, 0x57
    };
    ...
    ASSERT_BIN_ARRAYS_EQUALS(e, AWS_ARRAY_SIZE(e), private_key_data.publicExponent.ptr, private_key_data.publicExponent.len);
    ASSERT_BIN_ARRAYS_EQUALS(d, AWS_ARRAY_SIZE(d), private_key_data.privateExponent.ptr, private_key_data.privateExponent.len);
    ...
    ASSERT_BIN_ARRAYS_EQUALS(dmp1, AWS_ARRAY_SIZE(dmp1), private_key_data.exponent1.ptr, private_key_data.exponent1.len);
    ASSERT_BIN_ARRAYS_EQUALS(dmq1, AWS_ARRAY_SIZE(dmq1), private_key_data.exponent2.ptr, private_key_data.exponent2.len);
    ASSERT_BIN_ARRAYS_EQUALS(iqmp, AWS_ARRAY_SIZE(iqmp), private_key_data.coefficient.ptr, private_key_data.coefficient.len);
...
static int s_rsa_public_pkcs1_der_parsing(struct aws_allocator *allocator, void *ctx) {
    ...
    struct s_rsa_private_key_pkcs1 public_key_data = {0};
    ...
    ASSERT_BIN_ARRAYS_EQUALS(e, AWS_ARRAY_SIZE(e), public_key_data.publicExponent.ptr, public_key_data.publicExponent.len);
