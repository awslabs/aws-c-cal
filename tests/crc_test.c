...
static int s_test_known_crc(
    ...
    const struct aws_byte_cursor input_fst_half = {
        .len = (int)(len / 2),
        .ptr = input->ptr,
    };
    const struct aws_byte_cursor input_scnd_half = {
        .len = (int)(len - len / 2),
        .ptr = (input->ptr) + (len / 2),
    };
    struct aws_hash *crc1 = func(allocator);
    aws_hash_update(crc1, &input_fst_half);
    aws_hash_update(crc1, &input_scnd_half);
...
static int s_test_known_crc32(const char *func_name, crc_fn *func, struct aws_allocator *allocator) {
    ...
    res |= s_test_known_crc(func_name, func, "TEST_VECTOR", &input1, KNOWN_CRC32_TEST_VECTOR, allocator);
    return res;
}
