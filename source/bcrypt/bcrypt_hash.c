...
static void s_load_sha256_alg_handle(void) {
  ...
  BCryptOpenAlgorithmProvider(&s_sha256_alg, BCRYPT_SHA256_ALGORITHM,
                              MS_PRIMITIVE_PROVIDER, 0);
  assert(s_sha256_alg);
  DWORD result_length = 0;
  BCryptGetProperty(s_sha256_alg, BCRYPT_OBJECT_LENGTH,
                    (PBYTE)&s_sha256_obj_len, sizeof(s_sha256_obj_len),
                    &result_length, 0);
}

static void s_load_md5_alg_handle(void) {
  /* this function is incredibly slow, LET IT LEAK*/
  BCryptOpenAlgorithmProvider(&s_md5_alg, BCRYPT_MD5_ALGORITHM,
                              MS_PRIMITIVE_PROVIDER, 0);
  assert(s_md5_alg);
  DWORD result_length = 0;
  BCryptGetProperty(s_md5_alg, BCRYPT_OBJECT_LENGTH, (PBYTE)&s_md5_obj_len,
                    sizeof(s_md5_obj_len), &result_length, 0);
}
...
struct aws_hash *aws_sha256_default_new(struct aws_allocator *allocator) {
  ...
  uint8_t *hash_obj;
  aws_mem_acquire_many(allocator, 2, &bcrypt_hash,
                       sizeof(struct bcrypt_hash_handle), &hash_obj,
                       s_sha256_obj_len);
  ...
  NTSTATUS status = BCryptCreateHash(s_sha256_alg, &bcrypt_hash->hash_handle,
                                     ...
                                     (ULONG)s_sha256_obj_len, NULL, 0, 0);
...
struct aws_hash *aws_md5_default_new(struct aws_allocator *allocator) {
  ...
  NTSTATUS status =
      BCryptCreateHash(s_md5_alg, &bcrypt_hash->hash_handle,
                       bcrypt_hash->hash_obj, (ULONG)s_md5_obj_len, NULL, 0, 0);
