...
static int s_aes_test_encrypt_empty_input(struct aws_allocator *allocator, void *ctx) {
    ...
    uint8_t iv[] = {
       0xFB, 0x7B, 0x4A, 0x82, 0x4E, 0x82, 0xDA, 0xA6, 0xC8, 0xBC, 0x12, 0x51
    };

    uint8_t key[] = {
        0x20, 0x14, 0x2E, 0x89, 0x8C, 0xD2, 0xFD, 0x98, 0x0F, 0xBF, 0x34, 0xDE, 0x6B, 0xC8, 
        0x5C, 0x14, 0xDA, 0x7D, 0x57, 0xBD, 0x28, 0xF4, 0xAA, 0x5C, 0xF1, 0x72, 0x8A, 0xB6, 
        0x4E, 0x84, 0x31, 0x42
    };
    ...
    uint8_t aad[] = {
        ...
        0x2F, 0x47, 0x43, 0x4D, 0x2F, 0x4E, 0x6F, 0x50, 0x61, 0x64, 0x64, 0x69, 0x6E, 0x67, 0x5C, 
        0x22, 0x7D
    };
...
static int s_aes_test_decrypt_empty_input(struct aws_allocator *allocator, void *ctx) {
    (void)ctx;
    ...
    ASSERT_SUCCESS(aws_symmetric_cipher_decrypt(cipher, data_cur, &decrypted_buf));
    ASSERT_FAILS(aws_symmetric_cipher_finalize_decryption(cipher, &decrypted_buf));

    return AWS_OP_SUCCESS;
}
AWS_TEST_CASE(aes_test_decrypt_empty_input, s_aes_test_decrypt_empty_input)
