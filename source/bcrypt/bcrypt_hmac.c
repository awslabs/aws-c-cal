...
static void s_load_alg_handle(void) {
  ...
  BCryptOpenAlgorithmProvider(&s_sha256_hmac_alg, BCRYPT_SHA256_ALGORITHM,
                              MS_PRIMITIVE_PROVIDER,
                              BCRYPT_ALG_HANDLE_HMAC_FLAG);
  assert(s_sha256_hmac_alg);
  DWORD result_length = 0;
  BCryptGetProperty(s_sha256_hmac_alg, BCRYPT_OBJECT_LENGTH,
                    (PBYTE)&s_sha256_hmac_obj_len,
                    sizeof(s_sha256_hmac_obj_len), &result_length, 0);
}
...
struct aws_hmac *aws_sha256_hmac_default_new(struct aws_allocator *allocator,
  ...
  uint8_t *hash_obj;
  aws_mem_acquire_many(allocator, 2, &bcrypt_hmac,
                       sizeof(struct bcrypt_hmac_handle), &hash_obj,
                       s_sha256_hmac_obj_len);
  ...
  NTSTATUS status = BCryptCreateHash(
      ...
      (ULONG)s_sha256_hmac_obj_len, secret->ptr, (ULONG)secret->len, 0);
