...
void aws_set_md5_new_fn(aws_hash_new_fn *fn) { s_md5_new_fn = fn; }

void aws_set_sha256_new_fn(aws_hash_new_fn *fn) { s_sha256_new_fn = fn; }

void aws_hash_destroy(struct aws_hash *hash) { hash->vtable->destroy(hash); }
...
int aws_hash_finalize(struct aws_hash *hash, struct aws_byte_buf *output,
  ...
  if (truncate_to && truncate_to < hash->digest_size) {
    ...
    struct aws_byte_buf tmp_out_buf =
        aws_byte_buf_from_array(tmp_output, sizeof(tmp_output));
    tmp_out_buf.len = 0;
...
static inline int compute_hash(struct aws_hash *hash,
...
int aws_md5_compute(struct aws_allocator *allocator,
                    struct aws_byte_cursor *input, struct aws_byte_buf *output,
                    size_t truncate_to) {
  return compute_hash(aws_md5_new(allocator), input, output, truncate_to);
}

int aws_sha256_compute(struct aws_allocator *allocator,
                       struct aws_byte_cursor *input,
                       struct aws_byte_buf *output, size_t truncate_to) {
