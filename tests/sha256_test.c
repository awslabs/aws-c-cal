...
static int s_sha256_test_extra_buffer_space_fn(struct aws_allocator *allocator, void *ctx) {
    ...
    aws_byte_buf_init(&digest_size_buf, allocator, AWS_SHA256_LEN);
    aws_byte_buf_init(&super_size_buf, allocator, AWS_SHA256_LEN + 100);

    aws_sha256_compute(allocator, &input, &digest_size_buf, 0);
    aws_sha256_compute(allocator, &input, &super_size_buf, 0);

    ASSERT_TRUE(aws_byte_buf_eq(&digest_size_buf, &super_size_buf));
    ASSERT_TRUE(super_size_buf.len == AWS_SHA256_LEN);
