...
#else
static struct aws_hmac *aws_hmac_new_abort(struct aws_allocator *allocator,
                                           struct aws_byte_cursor *secret) {
  (void)allocator;
  (void)secret;
  abort();
...
struct aws_hmac *aws_sha256_hmac_new(struct aws_allocator *allocator,
                                     struct aws_byte_cursor *secret) {
...
void aws_hmac_destroy(struct aws_hmac *hmac) { hmac->vtable->destroy(hmac); }
...
int aws_sha256_hmac_compute(struct aws_allocator *allocator,
                            struct aws_byte_cursor *secret,
