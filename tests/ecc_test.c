...
static int s_ecdsa_p256_test_export_asn1_key_pair(struct aws_allocator *allocator, void *ctx) {
    (void)ctx;
    ...
    struct aws_ecc_key_pair *key_pair = aws_ecc_key_pair_new_from_asn1(allocator, &asn1_encoded_key);
    ASSERT_NOT_NULL(key_pair);

    size_t encoded_length = 0;
    ASSERT_SUCCESS(aws_ecc_key_pair_get_asn1_encoding_length(key_pair, &encoded_length));
    // ASSERT_TRUE(encoded_length == AWS_ARRAY_SIZE(asn1_encoded_key_raw));

    struct aws_byte_buf encoded_buffer;
    ASSERT_SUCCESS(aws_byte_buf_init(&encoded_buffer, allocator, encoded_length));

    ASSERT_SUCCESS(aws_ecc_key_pair_append_asn1_encoding(key_pair, &encoded_buffer));
    ASSERT_TRUE(encoded_buffer.len == encoded_length);
    ASSERT_BIN_ARRAYS_EQUALS(asn1_encoded_key_raw, encoded_length, encoded_buffer.buffer, encoded_length);
...
AWS_TEST_CASE(ecdsa_p256_test_export_asn1_key_pair, s_ecdsa_p256_test_export_asn1_key_pair)